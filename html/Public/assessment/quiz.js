...
(function($) {
    $.fn.jquizzy = function(settings) {
        ...
        contentFob = '<div class="description"><p style="color:#5B5B5B;margin-top:15px;margin-bottom:20px;font-size:130%;">&nbsp&nbsp&nbsp&nbsp这是美国宇航局开发的测评领导者天生性格的有效工具，一共14道题，30秒即可完成测试，并查看到结果。</br>&nbsp&nbsp&nbsp&nbsp请想象自己<B>正值20岁左右</B>。让自己在放松和自然的状态下快速完成对以下问题的选择。</p></div>',
        ...
        superContainer.find('#finish').click(function() {
            ...
            if(ded>0&&req>0)
			{
			  ...
			  window.title_fenxiang='我被NASA鉴定为绿色（亲和型）,你呢你呢你呢你呢~~~';
			  alert(window.title_fenxiang);
			}  
            else if(ded<0 && req>0)
			{
            	type_string=config.resultComments.type2;
				window.title_fenxiang='我被NASA鉴定为蓝色（创新型）,你呢你呢你呢你呢~~~';
				alert(window.title_fenxiang);
			}
            else if(ded>0 && req<0)
			{
            	type_string=config.resultComments.type3;
				window.title_fenxiang='我被NASA鉴定为黄色（包容型）,你呢你呢你呢你呢~~~';
				alert(window.title_fenxiang);	
			}
			else if(ded<0 && req<0)   
            { 
			  type_string=config.resultComments.type4;
			  window.title_fenxiang='我被NASA鉴定为橙色（执行型）,你呢你呢你呢你呢~~~';
			  alert(window.title_fenxiang);
			}
