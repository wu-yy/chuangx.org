...
class CourseController extends Controller
{
    ...
    function descscore($key, $num)
    {
        ...
        $list_course = $course->where($where)
            ->where('is_delect != 1')
            ->order('id desc')
            ->page($num, $course_nu)
            ->select();
    ...
    function count_score($key)
    {
        ...
        $nu_course = $course->where('is_delect != 1')
            ->order('nu desc')
            ->count();
    ...
    function gettagarr($id)
    {
        ...
        $where_arr['code'] = array(
            'like',
            $list['code'] . "%"
        );
        
        $list_arr = $tag->where($where_arr)
            ->where("is_delect != 1")
            ->select();
    ...
    function jie($key, $cid)
    {
        ...
        $list_zhangjie = $zhangjie->where($where_zhangjie)
            ->where('is_zhangjie != 0')
            ->order('is_id asc')
            ->select();
    ...
    function test()
    {
        echo 'tag_id:' . session('tag_id');
        
        echo '<br />';
        
        echo 'tag_id1:' . session('tag_id1');
    ...
    function an_course($cid)
    {
        ...
        if (empty($list)) {
            ...
            $list_course_tag_cid = $course->order('nu desc')
                ->page(0, $lctcn)
                ->where("id != $cid")
                ->field('id')
                ->select();
        ...
        $list_c = $course_tag->distinct(true)
            ->where($where_c)
            ->where("cid != $cid")
            ->page(1, $lctcn)
            ->field('cid')
