...
class MessageController extends Controller {
    ...
    {
        $this->weixin_mood('weixin_jifen','weixin','2016年8月30日 16::01','线上消费','100积分','200积分');
    }
    function weixin_mood($mood_type,$send_type,$time='',$type='',$number='',$amount='')
    {
        $user_list=getuser(110120119);
        ...
        if($send_type=='weixin')
        {
            if(empty($user_list['phone_openid']))
            {
                return false;
            }
            ...
            {
                $data=array(
                    'first'=>array('value'=>urlencode("恭喜你成功升级为会员"),'color'=>""),
                    ...
                    'remark'=>array('value'=>urlencode('您的会员到期时间为'.$number),'color'=>''),
                );
            ...
            {
                $data=array(
                    'first'=>array('value'=>urlencode("您好，以下课程将在".$time."分钟后开始"),'color'=>""),
                    'keyword1'=>array('value'=>urlencode($type),'color'=>''),
                    'keyword2'=>array('value'=>urlencode($time),'color'=>''),
                    'remark'=>array('value'=>urlencode('若因故无法参加,请及时联系老师'),'color'=>''),
                );
        ...
        if($send_type=='weixin')
        {
            ...
            $this->doSend($data, $user_list['phone_openid'], $template_id,$url);
        }
