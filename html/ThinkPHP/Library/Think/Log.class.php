...
class Log {
    ...
    static function write_search($message,$level=self::ERR,$type='',$destination='') {
        if(!self::$storage){
            $type 	= 	$type ? : C('LOG_TYPE');
            $class  =   'Think\\Log\\Driver\\'. ucwords($type);
            $config['log_path'] = C('LOG_PATH');
        ...
        if(empty($destination)){
            $destination = C('LOG_PATH').'search'.date('y_m_d').'.log';
        }
        self::$storage->write("{$level}: {$message}", $destination);
    }
    static function write_onclick($message,$level=self::ERR,$type='',$destination='') {
        if(!self::$storage){
            $type 	= 	$type ? : C('LOG_TYPE');
            $class  =   'Think\\Log\\Driver\\'. ucwords($type);
            $config['log_path'] = C('LOG_PATH');
        ...
        if(empty($destination)){
            $destination = C('LOG_PATH').'onclick'.date('y_m_d').'.log';
        }
        self::$storage->write("{$level}: {$message}", $destination);
    }
}